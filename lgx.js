...
async function main() {
  ...
  var reff = ''; // Kode Reff
  var useproxy = true; // true : pake proxy , false : gak pake
  var proxy = fs.readFileSync('proxy.txt').toString().split("\n");
while(true){
for (let i = 0; i < 8; i++) {
    ...
    return new Promise((resolve, reject) => {
     ...
     await nama.then(function(result) {
      ...
      async function getemail() {
         return new Promise((resolve, reject) => {
           superagent
             .post("https://mob2.10minemail.com/mailbox")
             .set({
               'Host': 'mob2.10minemail.com',
    ...
    var register = newreg();
     register.then(function(msg) {
      ...
      async function getmailbox(token) {
        return new Promise((resolve, reject) => {
          superagent
            .get("https://mob2.10minemail.com/messages")
            .set({
              'Host': 'mob2.10minemail.com',
              'Accept': 'application/json',
              'Content-Type': 'application/json',
              ...
              try {
                if (inbox[0] != '') {
                  ...
                  async function getbody(token,id) {
                    return new Promise((resolve, reject) => {
                      superagent
                        .get("https://mob2.10minemail.com/messages/"+id)
                        .set({
                          'Host': 'mob2.10minemail.com',
                          'Accept': 'application/json',
                          'Content-Type': 'application/json',
                  ...
                  body.then(function(isi) {
                    ...
                    superagent
                        ...
                        .set({
                          'Accept': '*/*',
                          "referer": "https://mob2.10minemail.com/messages/"+id,
